...
(def base
  [:html {:lang "en"}
   ...
   [:body.bg-slate-50.text-slate-800.min-h-screen.flex.flex-col
    ...
    [:main.flex-grow.flex.items-center.justify-center
     [:div.container.mx-auto.px-4.max-w-4xl.text-center
      [:h1.text-6xl.font-bold.mb-6.text-slate-900 "Welcome to " [:span.bg-gradient-to-r.from-emerald-400.via-sky-500.to-sky-300.bg-clip-text.text-transparent.relative "Clojure
          Stack Lite"]]
      [:p.text-xl.mb-10.text-slate-500 "A lightweight, modern template to jumpstart your Clojure projects"]
      [:p.text-lg.mb-12.text-slate-600 "To begin, add new routes in " [:code.bg-slate-200.px-1.rounded "src/yourproject/routes.clj"] " and define a handler in " [:code.bg-slate-200.px-1.rounded "src/yourproject/handlers.clj"]]
      [:div.mb-16
