...
(def base
  [:html {:lang "en"}
   ...
   [:body.text-slate-800.min-h-screen.flex.flex-col
    {:class ["bg-slate-50"]}
    [:nav.w-full.py-4.px-6.flex.justify-end
     [:a.text-slate-700.hover:text-slate-900 {:href "https://github.com/bogoyavlensky/clojure-stack-lite" :target "_blank" :rel "noopener noreferrer"}
    ...
    [:main.flex-grow.flex.items-center.justify-center
     [:div.container.mx-auto.px-4.max-w-4xl.text-center
      [:h1.text-6xl.font-bold.mb-6.text-slate-900 "Welcome to " [:span.bg-gradient-to-r.from-emerald-400.via-sky-400.to-sky-200.bg-clip-text.text-transparent.relative "Clojure
          Stack Lite"]]
      [:p.text-2xl.mb-10.text-slate-500 "A lightweight, modern template to jumpstart your Clojure projects"]
      [:p.text-lg.mb-12.text-slate-600
       "To begin, just change this view at " [:code.bg-slate-200.px-1.rounded "src/yourproject/views/home.clj"]
       " or add new route in " [:code.bg-slate-200.px-1.rounded "src/yourproject/routes.clj"]
       " and define a handler in " [:code.bg-slate-200.px-1.rounded "src/yourproject/handlers.clj"]]
      [:div.mb-16
       [:a.bg-slate-900.hover:bg-slate-800.text-white.font-medium.py-3.px-8.rounded-lg.transition-colors.duration-200.mr-4 {:href "https://github.com/bogoyavlensky/clojure-stack-lite"} "Get Started"]
       [:a.bg-white.hover:bg-slate-100.text-slate-900.font-medium.py-3.px-8.rounded-lg.border.border-slate-300.transition-colors.duration-200 {:href "https://github.com/bogoyavlensky/clojure-stack-lite/docs"} "Documentation"]]
      [:div.flex.flex-wrap.justify-center.gap-8.mb-8
       [:div.flex.flex-col.items-center
        [:svg.h-8.w-8.mb-2 {:viewBox "0 0 128 128" :xmlns "http://www.w3.org/2000/svg"}
         [:g {:fill "none"}
